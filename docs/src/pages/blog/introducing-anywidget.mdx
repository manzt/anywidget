---
title: Custom Jupyter Widgets Made Easy
...
and it is. The issue is that \*\*every widget author currently solves the

<u>same</u> multi-platform problem** with bundling, and the solution requires long-term
maintenance. Not only must the JavaScript tooling stay up to date with the fast-moving
...
2. Solve the same mutli-packaging problem
...
With ESM, the follow code runs in all major browsers without modifications:
...
model state between the widget fronend and the Python kernel (i.e., two-way data binding).
...
the front end relies, in the Jupyter Widgets [documentation](https://ipywidgets.readthedocs.io/en/8.0.2/examples/Widget%20Custom.html#Models-and-views).
