...
<iframe
  ...
  title="Anywidget fundamentals tutorial"
  frameborder="0"
...
- **Use the browser console**. View errors or intermediate values in your
  ...
  <center>
    <div class="my-3 px-1 w-11/12 max-w-lg pb-1 bg-primary-400/50 rounded border border-primary-400/50 shadow-md shadow-primary-500/50">
      <div class="flex items-center" aria-hidden="true">
        ...
        <div class="overflow-x-hidden flex grow items-center mr-4 space-x-2 col-span-4 rounded text-xs m-1 p-1 bg-white text-neutral-500 select-none">
          <svg
...
Here are some helpful things to know when working with widgets that aren't
always obvious.

### Always specify `names` when using `observe`

When observing changes to a widget in Python, it's tempting to write:
...
This fires `on_change` for _every_ traitlet change — including internal ones
you don't care about. Since multiple traits often update together, your callback
may fire several times per logical change, causing glitchy behavior.

Instead, always specify which trait(s) you care about:

```python
widget.observe(on_change, names=["value"])
```

### Use `ipywidgets.Output` to see errors

Errors raised inside `observe` callbacks in Jupyter environments are swallowed
silently. Wrap your logic with an `Output` widget to surface them:
...
def on_change(change):
    # errors will now display in `out`
    print(change.new)

widget.observe(on_change, names=["value"])
ipywidgets.VBox([widget, out])
```
