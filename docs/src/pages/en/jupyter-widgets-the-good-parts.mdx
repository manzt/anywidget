...
supports custom binary data as well. **anywidget** and `ipywidgets`
...
the model state. This ensures that you can safely pass binary data to the front
end without additional overhead (e.g., converting to JSON or base64 encoding).

Here's a summary of how Python data types are mapped to JavaScript types:
...
Alternatively, you can just use `traitlets.Any` to avoid validation, and
the data will sill be serialized to the front end according to the table above.
...
A custom serializer for a trait can be defined by adding a `to_json` method to
the metadata, which must return one of the types listed in the [Data
...
class Widget(anywidget.AnyWidget):
    _esm = """
    function render({ model, el }) {
      console.log(model.get("my_path"));  // { name: "example.txt", contents: DataView(13) }
    }
    export default { render };
    """
    my_path = traitlets.Instance(pathlib.Path).tag(sync=True, to_json=path_to_json)

Widget(my_path=pathlib.Path("example.txt"))
```

A more complex example might be to serialize a `pandas.DataFrame` to the
[Apache Arrow](https://arrow.apache.org/#) format or a `numpy` array to a
binary buffer. Several [community examples](./community) should serve as a good
starting point for learning more advanced use cases.
