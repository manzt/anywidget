...
A custom serializer for a trait can be defined by implementing `to_json` hook
on the trait metadata, which must return one of the types listed in the [Data
...
a good starting point to learn about more advanced use cases.
