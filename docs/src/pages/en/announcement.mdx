...
frontend tooling (e.g., `yarn`, Webpack, Babel, ESLint, Jest) and substantial
effort is _still_ required to keep the vendored build system up to
date with the latest platform requirements. Additionally, the recommended build systems
differ significantly between the official templates and come with
...
system (configured with `setup.py` + `setup.cfg` + `pyproject.toml`), whereas the TypeScript-based
cookiecutter the more modern [`hatch-jupyter-builder`](https://github.com/jupyterlab/hatch-jupyter-builder)
plugin for [Hatch](https://github.com/pypa/hatch) (configured with `pyproject.toml`)

<blockquote>

For the curious reader, I created separate projects for a `mywidget` package using the
two official cookiecutters and included the generated directories. Note the differences
in layout and number of files. 

<br/>

This section not intended to comment on the design choices by the Jupyter team, but to illustrate
the surface area a widget author is responsible for understanding and maintaining after project initialization.
For anyone who has used `create-react-app`, this is reminicant of running `react-script eject` and
vendoring all build scripts.
...
Due to the complicated nature of the recommended build systems and 

As a developer I have always found

As a developer, I should not need to create a new repository and local install 
to start prototyping my code. For python code, I can edit directly in my terminal.

JavaScript tooling has advanced in that development not longer requires a full  page refresh...
