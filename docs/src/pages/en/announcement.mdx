...
support for `.ipynb`. However, these tools vary with regard to support for extensions,
...
rolling a solution from scatch is [_very_ challenging to do correctly](https://twitter.com/trevmanz/status/1491815033734668288).
...
For anyone who has used `create-react-app`, it is reminicant of running `react-script eject` and
...
for Python objects in the Jupyter kernel. However, the proliferation of Jupyter-like
tools has resulted in a fragmented ecosystem that assumes reusable widgets are only possible 
if widget authors,

- Familarize themselves with frontend build tooling and packaging
...
employed by "Jupyter-like" applications. But what if this step wasn't 
...
Lastly, since **anywidget** bypasses the various platform-specific module systems infavor 
...
output notebook cells without refereshing the page or re-executing the a given cell.
...
to define custom rendering locic and initialize dynamic updates for the widget. This is an
...
<div style="background-color: lightseagreen; padding: 0px 2px;" >Hello, World</div>
...
the button is clicked. A second event handler is registered to update the the text 
output each time `value` changes on the model. 

By treating the model as the source of truth, whether Python kernel or JavaScript 
update `value`, the count displayed is correct. See the Jupyter Widgets [documentation](https://ipywidgets.readthedocs.io/en/8.0.2/examples/Widget%20Custom.html#Render-method) 
to learn more about the model view controller (MCV) framework the IPython widget frameowrk frontend relies on.
...
directly from a CDN via URL so as long as they are also ESM. Browsers native
...
**anywidget** does not currently allow the extension the `DOMWidgetModel`, so custom serialization 
and deserialization of traitlets is not currently supported. Additionally lifecycle hooks 
(e.g., `DOMWidgetView.remove()`) are not recognized, which can be useful for freeing browser 
resources when a widget view is removed. I am actively working to support both of these use cases
