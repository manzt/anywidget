...
[**anywidget**](https://github.com/manzt/anywidget), which <u>**greatly**</u>
simplifies creating and publishing custom Jupyter Widgets. Unlike the traditional
(cookiecutter) approach, with **anywidget** you don't have to fiddle with any
build steps or bundlers, you can prototype widgets from within a notebook,
and you get a modern frontend developer experience. **anywidget** allows you to write code
like this,_
...
class CounterWidget(anywidget.AnyWidget):
    # Widget frontend JavaScript code
    _esm = """
...
Colab, VS&nbsp;Code, and more. No messy build configuration or bundlers. It Just Works<sup>TM</sup>!_

<a href="#introducing-anywidget">Skip ahead.</a>
...
must **discover** the frontend widget code then **load** and **execute** the
JavaScript – a process which is not standardized, in contrast to the associated
...
and it largely is. The issue is that **every widget author has to solve the
...
work are used widely** - but to illustrate the large surface area a widget author is responsible for both understanding and maintaining after project initialization.
...
1. Familiarize themselves with frontend build tooling and packaging
2. Solve the same mutli-packaging problem for Jupyter-like targets
3. Create new packages to start prototyping
...
is a traditional custom widget whose sole purpose is to:
...
define ESM and an associated Python class (i.e., the components of a Jupyter Widget), and **anywidget** takes care of the rest.

A consequence of this design is that it drastically lowers the barrier to entry for many potential
widget developers The data scientist with minimal JavaScript experience can begin prototyping with her idea
for a domain-specific Jupyter integration directly within her notebook (and also learn frontend!). No need to first
install Node.js and understand frontend tooling. Conversely, a frontend engineer can learn Python
by turning one of their JavaScript libraries into a widget within Google Colab. No need to learn about
virtual environments or how Python modules work.

**anywidget** also adapts to a wide spectrum of use cases, enabling prototypes to incrementally
mature into robust Python packages. A student or hacker can save and share widgets via notebooks,
while an engineer can package and version the widgets he's created for his clients and co-workers.
As **anywidget** projects mature, JavaScript source can be moved from inline strings to separate files to
[advanced bundling](/en/bundling) when the complexity of the application demands it.
...
Custom widgets are defined by subclassing `anywidget.AnyWidget`.
...
- `_esm` the <u>**required**</u> ECMAScript module for the widget. It must must
...
- Custom stateful properties to synchronize with JavaScript and Python via [`traitlets`](https://ipywidgets.readthedocs.io/en/8.0.2/examples/Widget%20Custom.html#sync=True-traitlets)
  with `sync=True` metadata – just like regular Jupyter Widgets!
...
> without additionally installation or setup.
...
to frontend view. This example is slightly more sophisticated in that the
`render` function synchronizes the model state between the widget frontend and
Python kernel (i.e., two-way data binding). The `render` function creates a
`<button>` element and registers an event handler to increment the model `count` when
the button is clicked. A second event handler is registered to update the text
output each time `count` changes on the model.
...
about the model view controller (MCV) framework the IPython widget framework
frontend relies on in the Jupyter Widgets [documentation](https://ipywidgets.readthedocs.io/en/8.0.2/examples/Widget%20Custom.html#Models-and-views).
...
The project is still in its very early days and under active development. **It should _not_
yet be used in production since the API can change and some critial features are missing** 
(e.g., APIs to extend the `DOMWidgetModel` with custom serializers and `DOMWidgetView` with lifecycle hooks). Having 
said that, it's already [in use](https://github.com/vitessce/vitessce-python) and is ready for testing.
...
ideas to interactively explore biomedical datasets. If this is interesting or exciting to you, 
please reach out and get involved on GitHub! Happy coding.
