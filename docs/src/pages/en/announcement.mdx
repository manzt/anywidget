...
- `_esm` the **required** ECMAScript module for the widget. It must must export a `render`
  function to define custom rendering logic and initialize dynamic updates for the widget. This is an
...
This example is a reproduction of the starter widget from the [`juypyter-widgets/widget-ts-cookiecutter`](https://github.com/jupyter-widgets/widget-ts-cookiecutter/blob/master/%7B%7Bcookiecutter.github_project_name%7D%7D/src/widget.ts)
