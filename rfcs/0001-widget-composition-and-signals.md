...
The host MUST pass an `AbortSignal` to both `initialize` and `render`. The host MUST abort the signal when the widget model is destroyed (`initialize`) or when the view is removed (`render`).

New widgets SHOULD prefer `signal` over return callbacks for cleanup. Return callbacks MUST still be supported for backward compatibility, but `signal` is the recommended path going forward.

```js
export default {
  render({ model, el, signal }) {
    // DOM listeners SHOULD use { signal } for automatic cleanup
...
`initialize` MAY return an arbitrary object — the widget's **exports**. The host MUST store it and make it available via `getWidget`. The name mirrors VS Code's `Extension.exports` and ES module semantics: it's what the widget makes available for others to use.

The host MUST NOT impose constraints on the returned object. Widget authors define their own protocols, and consumers SHOULD use duck typing to check for support at the boundary. A slider widget might export `getValue`/`setValue`. A map widget might export `panTo(lat, lng)`. A data widget MAY export nothing at all. The contract is between the widget author and the consumer, not enforced by the framework.
...
| `object` | Widget exports (new; SHOULD use `signal` for cleanup) |

The host MUST distinguish these via `typeof ret === "function"`. Widgets that do not return an object MUST have `exports: undefined` when resolved. There is no implicit default — exports are strictly opt-in.
...
Any object that implements the anywidget protocol (has a `MimeBundleDescriptor` at `_repr_mimebundle_`) or extends `anywidget.AnyWidget` MUST be usable directly as a widget reference. No wrapper type or sentinel is needed — the serialization layer MUST auto-detect these objects in top-level state values during serialization and produce the appropriate wire format. This mirrors how `remove_buffers` already auto-detects binary types (`bytes`, `memoryview`) without explicit declaration.

Detection is top-level only — the serializer MUST NOT recurse into nested structures (dicts, lists) to find widget references. Nested and list-valued slots are deferred to a future RFC.
...
`_anywidget_slots_` is a reserved key and MUST NOT be used for other purposes. `model.get()` MUST NOT return slot references — `getWidget()` is the only way to access them.

### 4. `experimental.getWidget`

Resolves a child widget by trait name. The method MUST be async and MUST wait for the child's `initialize` to complete before resolving.

```js
async render({ model, el, signal, experimental }) {
  let child = await experimental.getWidget("header");

  // child.exports is `unknown` — consumers SHOULD use a type guard
  // to narrow before accessing methods.
...
When the parent passes its `signal` to `child.render`, the host MUST tie their lifecycles — aborting the parent's signal MUST cascade to the child's view teardown.
...
**Slot reassignment.** When a slot value changes at runtime (e.g., Python reassigns `dashboard.header = different_widget`), the host MUST fire `change:header` on the model. The parent SHOULD re-resolve via `getWidget` and SHOULD clean up previous child views via its own signal/abort logic.
...
**Why trait name, not raw ID?** Abstracts the wire format. Widget code never sees comm IDs or `_anywidget_slot` objects.

**Why no list slots?** `children: list[Slot]` requires diffing, keying, efficient updates. Deferred.

**Why auto-detect instead of an explicit wrapper type?** Objects that adhere to the anywidget protocol (have a `MimeBundleDescriptor`) or extend `AnyWidget` already carry enough information for serialization. Requiring an explicit `Slot(widget)` wrapper would add friction with no benefit — the serialization layer can detect these objects directly. `Slot` is provided only as a type alias for annotations.
...
1. Hosts MUST maintain a registry of widget models by comm ID.
2. Hosts MUST auto-detect anywidget-protocol objects during state serialization and produce `{ "_anywidget_slot": "<comm_id>" }`.
3. Hosts MUST map trait names to comm IDs when `getWidget` is called.
4. Hosts MUST wait for the child's `initialize` to complete before resolving `getWidget`.
5. Hosts MUST create an `AbortController` per view and MUST cascade abort when a parent view is torn down.

Hosts that do not yet support widget composition MUST still provide `getWidget` and SHOULD reject with a descriptive error:

```js
experimental.getWidget = () => Promise.reject(new Error("getWidget is not supported by this host."));
```
