...
await Deno.readTextFile(path.join(data_dir, "labextensions/anywidget/package.json"))
  .then((contents) => {
...
await Deno.remove(path.join(data_dir, "labextensions/anywidget"), {
  recursive: true,
}).catch(() => {});

await Deno.remove(path.join(data_dir, "nbextensions/anywidget"), {
  recursive: true,
}).catch(() => {});
