    ...
    }),
    {
      apply(/** @type {rspack.Compiler} */ compiler) {
        compiler.hooks.afterEmit.tap("CreateJupyterLabDataPlugin", (compilation) => {
          let entry = Object.keys(compilation.assets).find((f) => f.startsWith("remoteEntry."));
          if (!entry) {
          ...
          fs.writeFileSync(path.resolve(out, "package.json"), JSON.stringify(data, null, 2));
        });
      },
    },
    {
      apply(/** @type {rspack.Compiler} */ compiler) {
        compiler.hooks.afterEmit.tap("BundleNotebookPlugin", async () => {
